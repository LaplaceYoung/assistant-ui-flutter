...
const Map<String, Entry> _entries = <String, Entry>{
  ...
  'markdown-text': Entry('partial',
      ...
      note: 'fenced code runs through the syntax-highlighter tokenizer and the coldark palettes; ```mermaid fences render through the built-in painters (flowchart, pie, sequence), and inline/display math is typeset by components/math_renderer.dart (fractions, radicals, scripts, environments, accents, the blackboard letters, the font commands and the symbol table) with a host renderer overriding it. Constructs outside those subsets still fall back to the styled source'),
